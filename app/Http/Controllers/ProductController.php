...
class ProductController extends Controller
{
    ...
    public function product($slug, $tokenAfiliado = null)
    {
        ...
        if (!$productSlug) {
            // Slug inexistente: evita erro 500 e devolve o usuario para a lista de sorteios.
            return redirect()->route('sorteios')->with('error', 'Sorteio não encontrado.');
        }
    ...
    public function bookProductManualy(Request $request)
    {
        ...
        try {
            ...
            DB::rollBack();
            \Log::error('Falha ao reservar numeros: ' . $th->getMessage(), [
                'file' => $th->getFile(),
                'line' => $th->getLine(),
            ...
            return Redirect::back()
                ->withInput()
                ->withErrors('Não foi possível concluir sua reserva agora. Tente novamente em instantes.');
        }
    }
    ...
    public function mensagemWPPCompra($participanteID)
    {
        ...
        // Com o Baileys o envio não depende mais de token de terceiros:
        // basta a sessão estar conectada (QR lido em /wpp-mensagens).
        if ((new \App\Services\WhatsappBaileys())->conectado()) {
...
// ===== Envio via Baileys (painel WhatsApp multi-dispositivos) =====
// Antes: API de terceiros api.whatapi.com.br (token base64 + Bearer fixo).
// Agora: sessão própria conectada por QR Code em /wpp-mensagens.
$wppBaileys = new \App\Services\WhatsappBaileys();
$envioWpp = $wppBaileys->enviar($numero, $mensagem);
$response = json_encode($envioWpp);

if (!$envioWpp['ok']) {
    \Illuminate\Support\Facades\Log::warning('WhatsApp nao enviado para ' . $numero . ': ' . $envioWpp['erro']);
}
    ...
    public function gerarPIX(ModelsProduct $product, $resultPricePIX, $email, $name, $participante, $cpf, $telefone)
    {
        ...
        if ($product->gateway == 'mp') {
            if (empty($codeKeyPIX->key_pix)) {
                $response['error'] = 'Pagamento indisponível: o administrador ainda não configurou o token do Mercado Pago.';
                return $response;
            }
            ...
            try {
                ...
                \Log::error('Mercado Pago - token invalido: ' . $th->getMessage());
                $response['error'] = 'Pagamento indisponível: token do Mercado Pago inválido ou expirado. Atualize em Perfil > Integração.';
                return $response;
            }
            ...
            try {
                ...
                \Log::error('Mercado Pago - falha ao gerar PIX: ' . $th->getMessage());
                $response['error'] = 'Não foi possível gerar o PIX no momento. Tente novamente em instantes.';
                return $response;
            }

            $object = (object) $payment;

            $mpErro = isset($object->error->message) ? $object->error->message : null;

            if ($mpErro === 'payer.email must be a valid email') {
                $response['error'] = 'Erro ao gerar o QR Code!';
                return $response;
            }

            if ($mpErro === 'Invalid user identification number') {
                $response['error'] = 'CPF inválido!';
                return $response;
            }

            // Sem QR Code = credencial recusada ou pagamento nao criado.
            if (empty($object->id)
                || empty($object->point_of_interaction->transaction_data->qr_code)) {
                \Log::error('Mercado Pago - resposta sem QR Code: ' . json_encode($mpErro ?: $object->status));
                $response['error'] = $mpErro
                    ? ('Mercado Pago: ' . $mpErro)
                    : 'Pagamento indisponível: verifique o token do Mercado Pago no painel (Perfil > Integração).';
                return $response;
            }
    ...
    public function teste()
    {
        ...
        if (env('APP_ENV') == 'local') {
            $secretKey = env('MP_TOKEN_SANDBOX');   // token de teste: definir no .env
        } else {
